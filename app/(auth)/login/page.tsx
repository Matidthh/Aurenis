...
export default function LoginPage() {
  ...
  // Institución fijada manualmente
  ...
  const detectedInstitution = useMemo(() => {
    ...
    if (
      ...
      const prettyName = schoolPart.charAt(0).toUpperCase() + schoolPart.slice(1);
      return {
  ...
  async function handleQuickLogin(account: DemoAccount) {
    ...
    try {
      const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: account.identifier.trim(), password: account.pass }),
      });
