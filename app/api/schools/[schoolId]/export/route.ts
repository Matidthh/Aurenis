...
export async function GET(
  ...
  try {
    ...
    // 2. Authorization check (Autor: Carlos M.)
    if (!session) {
      return apiError("No autenticado. Inicie sesión para continuar.", "UNAUTHORIZED", { statusCode: 401 });
    }
    ...
    // 2.5 Rate Limiting para exportaciones masivas (Autor: Frank M. & Carlos M.)
    const rateLimitKey = `export:${session.userId}:${school.id}`;
    const rateLimit = await checkRateLimit(rateLimitKey, RATE_LIMIT_CONFIGS.BULK_EXPORT);
    if (!rateLimit.allowed) {
      return apiError(
        rateLimit.message || "Límite de solicitudes de exportación excedido. Por favor espere antes de intentar nuevamente.",
        "TOO_MANY_REQUESTS",
        {
          statusCode: 429,
          headers: {
