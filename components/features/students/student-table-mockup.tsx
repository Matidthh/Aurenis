...
export function StudentTableMockup({
  ...
  const effectiveStudents = useMemo(() => {
    return students && students.length > 0 ? students : MOCK_STUDENTS;
  }, [students]);

  // Filtros aplicados
  const filteredStudents = useMemo(() => {
    return effectiveStudents.filter((st) => {
      const matchSearch =
  ...
  }, [effectiveStudents, searchTerm, selectedCourse, selectedLevel, selectedStatus, onlyPie, sortBy, sortOrder]);
