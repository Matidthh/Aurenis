...
export const MOCK_STUDENTS: StudentMockupData[] = [
  ...
  },
  {
    ...
    guardianEmail: "claudia.munoz@yahoo.es",
    pie: false,
    scholarship: false,
  },
  {
    id: "st-6",
    rut: "23.701.892-2",
    name: "Fuentes Garrido, Florencia Antonella",
    course: "1° Medio B",
    level: "Media",
    avgGrade: 3.5,
    attendance: 78.0,
    status: "En Refuerzo",
    guardianName: "Patricia Garrido",
    guardianPhone: "+56 9 3210 9876",
    guardianEmail: "paty.garrido@gmail.com",
    riskFactor: "Riesgo de deserción escolar por ausentismo reiterado",
    pie: true,
    scholarship: true,
  },
];
...
/**
 * Fila de Estudiante Memoizada con React.memo
 * Responsable de autoría: Maicol R. (Módulo Alumnos) & Malcom Marcelo (Optimización)
 */
  ...
  return (
    ...
    >
      ...
      <td className="py-3.5 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-mono text-[11px] text-slate-400">{student.rut}</div>
          </div>
        </div>
      </td>
      ...
      <td className="py-3.5 px-4">
        ...
        <span className="text-[11px] text-slate-400">{student.level}</span>
      </td>

      <td className="py-3.5 px-4 text-center">
        <span
          className={`text-xs font-black px-2 py-0.5 rounded-md ${
            student.avgGrade < 4.0
              ? "bg-rose-100 text-rose-700 dark:bg-rose-950/80 dark:text-rose-300"
              : student.avgGrade < 5.0
        ...
        <div className="inline-flex flex-col items-center">
          <span
            className={`font-extrabold ${
              student.attendance < 85 ? "text-rose-600" : "text-slate-700 dark:text-slate-300"
            }`}
          >
            {student.attendance}%
          </span>
          <div className="w-12 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mt-0.5">
            <div
              className={`h-full ${
                student.attendance < 85 ? "bg-rose-500" : "bg-emerald-500"
              }`}
      ...
      <td className="py-3.5 px-4 text-right" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
          onClick={() => onSelectStudent(student)}
          className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-brand-600 transition cursor-pointer"
...
/**
 * Tarjeta de Estudiante Memoizada con React.memo
 */
export const StudentCardItem = memo(function StudentCardItem({
  student,
  onSelectStudent,
}: {
  student: StudentMockupData;
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 font-mono">{student.rut}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
        ...
        <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
          ...
          <span
            className={`text-sm font-black ${
              student.attendance < 85 ? "text-rose-600" : "text-slate-900 dark:text-white"
            }`}
          >
            {student.attendance}%
          </span>
...
export interface StudentTableMockupProps {
...
export function StudentTableMockup({
  ...
  const [onlyPie, setOnlyPie] = useState<boolean>(false);
  ...
  const filteredStudents = useMemo(() => {
    return students
      .filter((st) => {
        ...
        const matchesLevel = selectedLevel === "all" || st.level === selectedLevel;
        const matchesCourse = selectedCourse === "all" || st.course === selectedCourse;
        const matchesStatus = selectedStatus === "all" || st.status === selectedStatus;
        const matchesPie = !onlyPie || st.pie;

        return matchesSearch && matchesLevel && matchesCourse && matchesStatus && matchesPie;
      })
      .sort((a, b) => {
        let comp = 0;
        if (sortBy === "name") comp = a.name.localeCompare(b.name);
        else if (sortBy === "avgGrade") comp = a.avgGrade - b.avgGrade;
        else if (sortBy === "attendance") comp = a.attendance - b.attendance;
        return sortOrder === "asc" ? comp : -comp;
      });
  }, [students, searchTerm, selectedLevel, selectedCourse, selectedStatus, onlyPie, sortBy, sortOrder]);
  ...
  }, [selectedRowIds.length, filteredStudents]);
  ...
  const toggleSort = useCallback((field: "name" | "avgGrade" | "attendance") => {
    setSortBy((prevSort) => {
      if (prevSort === field) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {isLoading && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 text-[11px] font-semibold">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {onRefresh && (
            <button
              ...
              onClick={onRefresh}
              disabled={isLoading}
              title="Recargar desde la API REST"
              className="inline-flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 text-slate-700 dark:text-slate-200 transition shadow-xs disabled:opacity-50 cursor-pointer"
            >
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-extrabold bg-brand-600 hover:bg-brand-700 text-white shadow-md shadow-brand-500/20 transition cursor-pointer ${
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            <select
              ...
              className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:outline-hidden font-medium cursor-pointer"
            >
            ...
            <button
              ...
              className={`px-3 py-2 rounded-xl border font-semibold transition cursor-pointer ${
                onlyPie
          ...
          <div className="p-2.5 rounded-xl bg-brand-50 dark:bg-brand-950/60 border border-brand-200 dark:border-brand-800 flex items-center justify-between text-xs animate-in fade-in">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium cursor-pointer"
              >
                Deseleccionar
              </button>
            </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
                {filteredStudents.map((st) => (
                  <StudentTableRow
                    key={st.id}
                    student={st}
                    isSelected={selectedRowIds.includes(st.id)}
