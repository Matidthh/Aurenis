...
export function GradeMatrixSpreadsheet() {
  ...
  // Función de cálculo de promedio (Simple o Ponderado)
  const calculateStudentAverage = useCallback(
    (student: StudentRow) => {
      if (calcMode === "simple") {
        const validGrades = assessments
          .map((ass) => student.grades[ass.id])
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="bg-white dark:bg-slate-900 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-xs">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-between md:justify-end">
          {/* Selector de Modo de Promedio (Simple vs Ponderado) */}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 hover:bg-blue-100 dark:bg-blue-950/60 dark:text-blue-300 border border-blue-200 dark:border-blue-800 transition shadow-2xs"
          ...
          <thead>
            <tr className="bg-slate-100/90 dark:bg-slate-850 border-b border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-center gap-1">
                      ...
                      >
                        {calcMode === "simple" ? "Equitativo (1/N)" : `${ass.weightPct}%`}
                      </button>
                    </div>
              ...
              <th
                ...
                title={`Cálculo actual: ${
                  calcMode === "simple"
                    ? "Promedio Aritmético Simple (Suma de notas / Cantidad de notas)"
                    : "Promedio Ponderado Decreto 67 (Suma de [Nota × %] / 100%)"
                }. Haz clic para ver la guía y ejemplos.`}
              >
                <div className="flex items-center justify-center gap-1">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 flex items-center justify-center font-black text-lg">
                  <Calculator className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-white">
                    Lógica de Cálculo de Calificaciones & Ponderaciones
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Guía docente según la normativa nacional del Decreto 67/2018 del Mineduc
                  </p>
                </div>
              </div>
              <button
                onClick={() => setShowDecretoModal(false)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div
                className={`p-4 rounded-2xl border transition ${
                  calcMode === "simple"
                    ? "bg-blue-50/60 dark:bg-blue-950/40 border-blue-300 dark:border-blue-700 ring-2 ring-blue-400/20"
              ...
              >
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 mb-2.5 leading-relaxed">
                  <strong>Todas las notas valen exactamente lo mismo (1:1)</strong>. Es la regla estándar y más común en los establecimientos de Chile.
                </p>
                <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-300 space-y-1">
                  <div className="text-slate-400 text-[10px] font-sans font-bold">Fórmula:</div>
                  <div>Promedio = (N1 + N2 + ... + Nn) ÷ N</div>
                </div>
              </div>
              ...
              >
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 mb-2.5 leading-relaxed">
                  <strong>Cada evaluación tiene un peso porcentual (%)</strong> asignado según el RIE del colegio (ej: Controles 20%, Taller 15%, Examen 25%).
                </p>
                <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-300 space-y-1">
                  <div className="text-slate-400 text-[10px] font-sans font-bold">Fórmula:</div>
                  <div>Promedio = Σ(Nota × %Peso) ÷ Σ(%Pesos)</div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3">
              ...
              <div className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Supongamos que un alumno tiene 3 evaluaciones:
                <div className="grid grid-cols-3 gap-2 mt-2 font-mono text-center">
                  <div className="p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] text-slate-400 font-sans font-bold">Control 1 (20%)</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">6.0</div>
                  </div>
                  <div className="p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] text-slate-400 font-sans font-bold">Taller (30%)</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">5.0</div>
                  </div>
                  <div className="p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] text-slate-400 font-sans font-bold">Examen (50%)</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">4.0</div>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 font-sans">
                  <div className="p-2.5 rounded-xl bg-blue-50/80 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900">
                    <div className="font-bold text-blue-900 dark:text-blue-200 text-[11px]">En Promedio Simple:</div>
                    <div className="text-[11px] text-slate-600 dark:text-slate-300 mt-0.5">
                      (6.0 + 5.0 + 4.0) ÷ 3 = <strong className="text-blue-700 dark:text-blue-300 text-xs">5.0</strong>
                    </div>
                  </div>

                  <div className="p-2.5 rounded-xl bg-purple-50/80 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-900">
                    <div className="font-bold text-purple-900 dark:text-purple-200 text-[11px]">En Promedio Ponderado:</div>
                    <div className="text-[11px] text-slate-600 dark:text-slate-300 mt-0.5">
                      (6.0×0.2) + (5.0×0.3) + (4.0×0.5) = <strong className="text-purple-700 dark:text-purple-300 text-xs">4.7</strong>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Selector Rápido y Cierre */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <span className="text-xs text-slate-500 font-medium">Modo activo actual:</span>
                <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
                  <button
                    onClick={() => setCalcMode("simple")}
              ...
              <button
                onClick={() => setShowDecretoModal(false)}
                className="w-full sm:w-auto py-2.5 px-6 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-xs transition cursor-pointer shadow-xs"
              >
                Cerrar Guía
              </button>
            </div>
