...
export const MOCK_TEACHERS: TeacherData[] = [
  ...
  },
  {
    id: "tch-7",
    rut: "18.234.567-9",
    name: "Patricio Morales Vega",
    email: "p.morales@colegiosanjose.cl",
    phone: "+56 9 2190 5566",
    specialty: "Ingeniero en Computación e Informática y Docente Técnico (UTFSM)",
    department: "Tecnología & Formación",
    contractHours: 34,
    assignedHours: 6,
    status: "Activo",
    planningProgress: 94,
    digitalSignatureActive: true,
    subjects: [
      { id: "sub-21", name: "Tecnología & Robótica", course: "8° Básico A", weeklyHours: 2, room: "Lab Computación" },
      { id: "sub-22", name: "Tecnología & Robótica", course: "1° Medio B", weeklyHours: 2, room: "Lab Computación" },
      { id: "sub-23", name: "Pensamiento Computacional", course: "3° Medio B", weeklyHours: 2, room: "Lab Computación" },
    ],
  },
];
...
export function TeacherManagementMockup({
  ...
  const filteredTeachers = useMemo(() => {
    return MOCK_TEACHERS.filter((tch) => {
      const matchSearch =
  ...
  }, [searchTerm, selectedDepartment, selectedStatus, onlyHeadTeachers]);

  // Resumen de Métricas del Plantel
  const totalContractHours = MOCK_TEACHERS.reduce((acc, t) => acc + t.contractHours, 0);
  const totalAssignedHours = MOCK_TEACHERS.reduce((acc, t) => acc + t.assignedHours, 0);
  const avgPlanning = Math.round(
    MOCK_TEACHERS.reduce((acc, t) => acc + t.planningProgress, 0) / MOCK_TEACHERS.length
  );
  ...
  const activeFilterCount =
    ...
    (onlyHeadTeachers ? 1 : 0);

  function handleClearFilters() {
    setSearchTerm("");
    setSelectedDepartment("all");
    setOnlyHeadTeachers(false);
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-brand-500"
            />
            {searchTerm && (
              <button
                type="button"
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            ...
            {/* Botón de limpiar filtros operativo */}
            {hasActiveFilters && (
              <button
                type="button"
                onClick={handleClearFilters}
                className="px-3 py-2 rounded-xl text-xs font-bold text-red-600 dark:text-red-400 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950/40 border border-red-200 dark:border-red-900/60 flex items-center gap-1.5 transition"
            ...
            <div className="flex border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden ml-auto">
              <button
      ...
      {viewMode === "cards" ? (
        filteredTeachers.length === 0 ? (
          <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
            <EmptyState
              ...
              description="No hay docentes en la nómina que coincidan con los criterios de búsqueda o departamento seleccionado."
              ...
              action={
                <button
                  onClick={onOpenNewTeacherModal}
                  className="px-3.5 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-xs transition"
                >
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 px-1 lg:hidden">
            <span className="flex items-center gap-1 font-medium">
              ↔️ Desliza horizontalmente para ver todos los datos docentes
            </span>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl hover:bg-brand-50 hover:text-brand-600 dark:hover:bg-slate-800 text-slate-400 transition"
                        ...
                        <button
                          ...
                          className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl hover:bg-slate-100 hover:text-slate-800 dark:hover:bg-slate-800 text-slate-400 transition"
                          title="Editar Perfil"
                        >
