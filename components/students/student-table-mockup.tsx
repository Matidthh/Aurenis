...
export function StudentTableMockup({
  ...
  }, [searchTerm, selectedCourse, selectedLevel, selectedStatus, onlyPie, sortBy, sortOrder]);
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          {/* Campo Búsqueda con botón de limpiar */}
      ...
      {viewMode === "table" ? (
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 px-1 lg:hidden">
            <span className="flex items-center gap-1 font-medium">
              ↔️ Desliza horizontalmente para ver todos los datos del estudiante
            </span>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
                {filteredStudents.length === 0 ? (
                  <TableEmptyState
                    ...
                    description="No hay estudiantes matriculados que coincidan con la búsqueda o los filtros aplicados."
                  ...
                  filteredStudents.map((st) => (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => onSelectStudent(st)}
                          className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-brand-600 transition"
        ...
        filteredStudents.length === 0 ? (
          <div className="p-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
            <EmptyState
              title="No se encontraron estudiantes"
              description="No hay estudiantes matriculados que coincidan con la búsqueda o los filtros aplicados."
              searchTerm={searchTerm}
      ...
      <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs text-slate-500">
        <span>
          {filteredStudents.length === 0
            ? "0 estudiantes encontrados con los filtros actuales"
            : `Mostrando ${filteredStudents.length} de 842 estudiantes matriculados`}
        </span>
        <div className="flex items-center gap-2">
          <button
            disabled={filteredStudents.length === 0}
            className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="font-bold text-slate-800 dark:text-slate-200">
            Página {filteredStudents.length === 0 ? "0 de 0" : "1 de 1"}
          </span>
          <button
            disabled={filteredStudents.length === 0}
            className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition"
